...
local function updateProgress()
  ...
  local currMoney = career_modules_playerAttributes.getAttribute(questType).value

  for _, quest in ipairs(career_modules_questManager.getQuestsOfType(questType)) do  
    local goal = quest.type.conditions.moneyCount
    ...
    quest.tempData.progress = { {
        ...
        currValue = currMoney,
        maxValue = goal,
        label = {txt = "quest.type."..questType..".progess", count = currMoney, goal = goal},
      }
    }
